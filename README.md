...
- `valueCards/{id}` (auto-id) — same shape as `fanFavourites`, for the About page's value cards. The page's 4th card (100% Halaal certification) is hardcoded in `about.html`, not part of this collection — it's a certification fact, not marketing copy, so it isn't admin-editable.
...
- `dailySpecials/{monday..saturday}/items/{id}` (no `sunday` — shop is closed) — `item`, `promo`, `imageUrl`, `order`. Each day is a **subcollection**, not a single doc, so more than one special can run the same day and specials can be added/removed freely — same shape/pattern as `menuItems`. (The old `dailySpecials/{day}` doc itself may still carry legacy top-level `item`/`promo`/`imageUrl` fields from before this changed — harmless, unread by any current code once migrated, see below.)
...
**Admin panel** (`/admin.html`, not linked from the public nav): email/password login (Firebase Auth, one account created directly in the Firebase console under Authentication → Users), then six tabs — **Menu** (edit name/price/description per item, upload a photo to Cloudinary, add/delete items), **Site Text** (grouped by page, each group saves independently), **Fan Favourites** and **Value Cards** (add/edit/delete cards — `js/admin.js`'s `makeCardCollectionEditor()` factory builds both, since they're the identical shape), **Business Info** (the centralized phone/address/hours/email/links), **Daily Specials** (add/edit/delete any number of specials per weekday). An "Import Starter Data" button appears only when `menuCategories` is empty, so it can't accidentally be re-run over real edits later.
...
These three moved from a fixed shape (a set number of siteContent fields; one special per day) to real collections the admin panel can freely add to or remove from. The real content that existed before this change (4 Fan Favourites, 3 Value Cards, 6 days of specials — including the real photos already uploaded for Monday and Tuesday's specials) is **not yet migrated into the new collections** — that happens automatically, once, the next time the owner logs into `/admin.html` (`migrateFixedSiteContentCards()` / `migrateDailySpecialsIfNeeded()` in `js/admin.js`, called from `loadEverything()`). Until then, every public page keeps showing the real existing content exactly as it always has — confirmed directly against live production data (not just reasoned about): the old `siteContent` fields and the old per-day `dailySpecials/{day}` docs still have all the real values, and the new collections are empty, so `site-content.js`/`js/promo.js`'s fallback logic keeps the current real cards/specials (including both existing special photos) showing with zero visual change until migration runs.
