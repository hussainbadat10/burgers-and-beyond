...
js/escape-utils.js Shared escapeHtml()/escapeAttr() — every file that builds
                    HTML via string concatenation imports these instead of
                    redefining its own copy (see below)
...
- `valueCards/{id}` (auto-id) — same shape as `fanFavourites`, for the About page's value cards, plus an optional `imageUrl` (a real photo/logo takes priority over the emoji when both are set — used for the 100% Halaal card's real SANHA logo). That card was originally hardcoded/non-editable by deliberate design (a certification fact, not marketing copy); reversed per hbadat — it's now just another card in this collection, fully add/edit/delete-able like the rest. Any uploaded photo — a food shot for a Fan Favourite, a certification badge like the SANHA logo, anything — renders in the same 72x72 box as the emoji it replaces (`.card-photo` in `css/style.css`), regardless of the uploaded image's own dimensions: `object-fit: cover` crops to fill the box rather than scaling to fit inside it. (An earlier version used `contain` specifically to avoid ever cropping the Halaal logo's text — but that meant a real-world portrait phone photo, very common for Fan Favourite uploads, rendered as a narrow letterboxed sliver instead of filling the box like every other card. `cover` was checked directly against the Halaal logo before switching: it only trims the decorative ribbon tips, the seal and "S.A.N.H.A." text stay fully intact.)
...
**Admin panel** (`/admin.html`, not linked from the public nav): email/password login (Firebase Auth, one account created directly in the Firebase console under Authentication → Users), then four tabs mirroring the site's own page structure, each ordered top-to-bottom exactly like the real page it edits — **Home** (hero/reviews/footer site text, then Fan Favourites cards, then the site-wide Branding logo/Halaal badge — see below), **Menu** (menu hero text, then Daily Specials, then Menu Categories/Items — Daily Specials genuinely comes first here since the real menu page shows the specials banner above the category list), **About** (hero + story site text, then Value Cards including the 100% Halaal card, then the Bottom CTA text — Value Cards sit between the story and the CTA on the real page, so the admin tab does too), **Contact** (contact hero text, then Business Info — the centralized phone/address/hours/email/links). Menu Categories support add/edit/delete (each with its own emoji/name/note), Menu Items support add/edit/delete/photo per item, Fan Favourites and Value Cards share `js/admin.js`'s `makeCardCollectionEditor()` factory (add/edit/delete cards, identical shape). Each site-text group saves independently — `CONTENT_GROUPS` (in `js/admin.js`) drives them; each group carries a `page` field used to route it into the right container (About's Bottom CTA deliberately uses a distinct `'about-cta'` page value from the rest of About's groups, specifically so it can render in a separate container placed after Value Cards), but `data-group-index` on the rendered element always points at the group's position in the single global array, so `saveContentGroup()` doesn't need to know which container it was rendered into. An "Import Starter Data" button appears only when `menuCategories` is empty, so it can't accidentally be re-run over real edits later.

**Menu Categories** (`js/admin.js`'s `saveCategory()`/`deleteCategory()`/`addCategory()`): unlike every other collection, `menuCategories` documents use a human-readable slug as their Firestore document id (e.g. `smash-burgers`), not an auto-generated one — `addCategory()` derives one from the typed name (lowercased, hyphenated, de-duplicated against existing ids if there's a clash) via `slugifyCategoryName()`. Only `name`/`emoji`/`note` are editable from the UI; the rarer `comboCallout` (raw HTML) and `dividerBefore` (the "Sharing Meals" section-divider object) stay Firestore-console-only, since `updateDoc` only touches the fields it's given and never wipes them. Deleting a category is blocked with a toast (not silently allowed) while it still has menu items in it, so items can never be orphaned — delete or move its items first.
...
Audited with Lighthouse (`npx lighthouse`, against a local build of `_site/`, all 7 pages). Home page went from 71/94/100/92 (performance/accessibility/best-practices/SEO) to 88/100/100/100; every other page is at 100/100 for accessibility/SEO except 404.html's SEO score (58 — see below, this is correct, not a bug). Fixed:

- **Heading hierarchy**: the home page's info-strip (Opening Hours/Collection Point/Order By Phone/Order Delivery) and the contact page's "Order & Collection" both jumped straight from `<h1>` to `<h3>`/`<h4>`, skipping `<h2>` — fixed to `<h2>`. The footer's "Explore"/"Visit Us"/"Hours" (shared across every page) were `<h4>` regardless of what heading level preceded them on a given page; now `<h2>` with a `.footer-heading` class doing the styling instead of the tag, since the footer is its own landmark and can reasonably restart its own heading level. The footer logo's "Burgers N Beyond" was demoted from a heading to a plain `<p>` — it's a brand label next to a logo, not a section introducing content, same as the nav logo's text was never a heading either.
- **Color contrast**: `.section-eyebrow` (`#2572b3`, 4.19:1) and the light-background "Open Now" badge (`#1f8a3b`, 3.79:1) both fell just short of WCAG AA's 4.5:1 for their text size — darkened both individually rather than touching the shared `--color-accent` variable everywhere it's used.
- **Descriptive link text**: the cookie banner's "Learn more" is now "Learn more in our Privacy & Cookies policy".
- **Explicit image dimensions**: the nav/footer logo and the Halaal badge images only constrained one CSS dimension (e.g. `height: 38px; width: auto`), so the browser can't reserve their layout space until the stylesheet loads. Added matching `width`/`height` HTML attributes; same for the dynamic menu-item/promo/card photos, which already had both CSS dimensions fixed but benefit from the same reservation before CSS is parsed.
- **`images/sanha-logo.png`** resized from 378×250 (58KB) to 220×146 (37KB) — its largest real use is the 72px Halaal value card, so the original was still oversized (same class of fix as `images/icon.png` earlier).
- **Cloudinary upload transform** (`optimizedCloudinaryUrl()` in `js/admin.js`) lowered from `w_500` to `w_250` — no photo on the site displays wider than 80px even at 3x retina (240px), so 500px-wide originals were pure waste. Only affects photos uploaded from now on; already-uploaded photos keep whatever width was baked into their stored URL when they were uploaded, since Cloudinary transforms are part of the URL itself.
- **Render-blocking Google Fonts stylesheet**: was a plain blocking `<link>`; now loaded via the standard `media="print" onload="this.media='all'"` async pattern (with a `<noscript>` fallback), plus added missing `preconnect` hints for `fonts.gstatic.com` (the actual font file host, separate from the CSS host) and `www.gstatic.com` (Firebase's SDK host).

**404.html's SEO score (58) is intentional, not a bug**: Lighthouse flags it for being blocked from indexing and having no meta description, but 404 pages are *supposed* to have `<meta name="robots" content="noindex">` — you never want a search engine indexing an error page as if it were real content. Don't "fix" this.

**Not fixed, bigger architectural lift**: unminified/unbundled JS (this project deliberately has no JS build step — see the top of this file) and the ~100KB of unused code inside Firebase's own SDK bundles (would need tree-shaking, which means introducing a JS bundler). GitHub Pages' cache lifetimes (`max-age=600`) also show up as a Lighthouse finding — that's a hosting-platform setting, not something controllable from this repo; a host with configurable headers (relevant if this ever moves to Afrihost) could improve it.
...
- **OG share image**: `images/og-image.png` is logo-only (no real food photo exists yet) — worth swapping for a real hero food shot once photography is done. **Deliberately not admin-editable, unlike the logo/Halaal badge below** — the `<meta property="og:image">` tag is baked into the static HTML at build time, and link-preview crawlers (WhatsApp, Facebook, etc.) read that raw HTML directly without running any JavaScript, so a value fetched from Firestore after page load would never actually reach them. Changing it for real means replacing the file in `images/` and rebuilding/redeploying — ask for that whenever a real photo is ready, it's a one-line change.
